...
class BARsaveEditor(tk.Tk):
    def __init__(self):
        ...
        self.btn_load = ttk.Button(
            frame_controls, text="Load save.bin", command=self.load_file
        )
        ...
        self.btn_export_txt = ttk.Button(
            frame_controls,
            text="Export records to TXT files",
            command=self.export_records_txt,
            state="disabled",
        )
        ...
        self.tree = ttk.Treeview(
            tree_frame, columns=("index", "value"), show="headings", height=25
        )
    ...
    def validate_values(self):
        ...
        if not (0 <= search_val <= MAX_VALUE):
            messagebox.showerror(
                "Value error", f"Value to find must be between 0 and {MAX_VALUE}."
            )
            return None, None
        if not (0 <= replace_val <= MAX_VALUE):
            messagebox.showerror(
                "Value error", f"Replacement value must be between 0 and {MAX_VALUE}."
            )
            return None, None
    ...
    def load_file(self):
        filepath = filedialog.askopenfilename(
            title="Open save.bin", filetypes=[("BIN files", "*.bin")]
        )
        if not filepath:
            return
        ...
        try:
            ...
            self.integers = list(
                struct.unpack(f"<{num_values}I", self.file_content[: num_values * 4])
            )
            ...
            if detected_value is not None:
                ...
                self.status_var.set(
                    f"Detected value {detected_value} at position {detected_index}."
                )
            else:
                self.entry_search.delete(0, tk.END)
    ...
    def save_file(self):
        ...
        if count_replaced == 0:
            messagebox.showinfo(
                "Info", f"No occurrences of value {search_val} found to replace."
            )
        ...
        try:
            ...
            self.status_var.set(
                f"File saved successfully: {filepath} ({count_replaced} values replaced)."
            )
            messagebox.showinfo(
                "Saved", f"File saved successfully. {count_replaced} values replaced."
            )
        except Exception as e:
    ...
    def export_records_txt(self, ask_path=True):
        ...
        if ask_path:
            ...
            if self.filepath is None:
                messagebox.showerror(
                    "No file path", "Load a file first to auto-export."
                )
                return
        ...
        for start_index in range(0, total_records, MAX_DISPLAY_RECORDS):
            ...
            try:
                ...
                messagebox.showerror(
                    "Export error", f"Error saving file {filename}:\n{str(e)}"
                )
                break
        ...
        if files_saved > 0:
            ...
            if ask_path:
                messagebox.showinfo(
                    "Exported", f"Exported {files_saved} TXT files with records."
                )
        else:
